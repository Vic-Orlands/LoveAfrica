...
const Facebook = () => {
	...
	const facebookLogin = async () => {
		try {
			...
			const { type, token } = await FacebookLogin.logInWithReadPermissionsAsync({
	...
	return LoggedIn ? userData ? (
		<View style={styles.container}>
			...
			/>
			<Text style={[ { fontFamily: 'Bold' }, tw`underline text-xl` ]}>Hello {user}</Text>

			<View style={tw`mt-24`}>
				<TouchableOpacity style={tw`bg-gray-600`} onPress={logout}>
					<Text style={[ { fontFamily: 'Bold' }, tw`p-2` ]}>Logout</Text>
				</TouchableOpacity>
			</View>
		</View>
	) : null : (
		<View style={styles.container}>
			<TouchableOpacity style={tw`mt-12 bg-blue-300`} onPress={set}>
				<Text style={[ { fontFamily: 'Bold' }, tw`p-2` ]}>Login to my Network</Text>
			</TouchableOpacity>
		</View>
	);
};
